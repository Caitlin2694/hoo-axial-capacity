<template>
  <v-container class="mt-5 background">
    <v-card>
        ...
        <v-col cols="7">
          <div>
            <results-graph
              class="ma-5"
              :result="chart_data"
              xaxis="Axial capacity (kN)"
              yaxis="Pile tip depth (m)"
              height="500"
            ></results-graph>
          </div>
        </v-col>
        <v-col cols="5">
          <v-simple-table fixed-header class="mt-5" height="300px">
            <template v-slot:default>
              ...
              <tbody>
                <tr v-for="item in data_dict" :key="item.tipdepth">
                  <td>{{ item.tipdepth }}</td>
                  <td>
                    {{
                      item.tension_capacity
                        ? item.tension_capacity.toFixed(0)
                        : "N/A"
                    }}
                  </td>
                  <td>
                    {{
                      item.compression_capacity
                        ? item.compression_capacity.toFixed(0)
                        : "N/A"
                    }}
                  </td>
                </tr>
      ...
      <div class="ma-5">
        <p>
          Thankyou for using our axial capacity calculator app! Click "back to
          input" to try different input data. Otherwise, click "go home" to go
          back to the home page.
        </p>
        <p class="text-body-2 mt-3 ml-1">
          This freeware is provided "AS IS", and you, its user, assume all risks
          when using it. Licenced under a
          <a href="/licence" target="_blank">MIT license</a>
        </p>
        <v-row>
          <v-col>
            <v-btn class="ma-2 mb-5" color="primary" @click="navInput()">
              Back to Input
            </v-btn>

            <v-btn class="ma-2 mb-5" color="primary" @click="navHome()">
              Go Home
            </v-btn>
          </v-col>
          <!--<v-col>
        ...
        <v-row>
          ...
          >
            <v-btn class="ma-2 mb-5" color="primary">
              Debug Params
            </v-btn>
          </download-csv>
...
export default {
  ...
  methods: {
    ...
    navInput() {
      this.$router.push({ path: "/" });
      if (this.alg_type == "driven")
        this.$router.push({ path: "/calculator?pile=driven" });
